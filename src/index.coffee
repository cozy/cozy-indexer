...
prepareField = (value, stemmed, facet, includeObjectKeys) ->
    ...
    unless value
        ...
        value = stem value if stemmed
        value = [value.toLowerCase()] if facet

    else if Array.isArray(value)
        value = value.map (part) -> prepareField part, stemmed
        ...
        for k, v of value
            ...
            joined.push prepareField v, stemmed
...
makeIndexableDoc = (doc, options) ->
    ...
    for own field, value of doc
        ...
        indexable[field] = prepareField value, isStem, isFacet
...
# @return {object} the document prepared.
###
exports.addBatch = (docs, fieldOptions, callback) ->

    return callback null if docs.length is 0

    options = makeBatchOptions fieldOptions
    batch = []
    for doc in docs
        indexable = makeIndexableDoc doc, options
        if indexable then batch.push indexable
        else console.log """
            ignoring doc without indexable field #{doc._id}
        """

    if batch.length then si.add batch, options, callback
...
exports.store =
    open: (callback) ->
        checkIfOpen = ->
            return callback null if si.indexes.isOpen()
            setTimeout checkIfOpen, 100
        checkIfOpen()
    set: (key, value, callback) ->
        si.indexes.put 'KV￮' + key + '￮', value, callback
    get: (key, callback) ->
        si.indexes.get 'KV￮' + key + '￮', callback
...
# @returns {object} olddef if there is no need to change it, a mix of both
                    otherwise
###
exports.mergeFieldDef = (olddef, newdef) ->
